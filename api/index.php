...
$app->post('/userUpdate','userUpdate'); /* email, Username, pwd Updated in admin panel*/
...
function signup() {
    ...
    try {
        ...
        if (strlen(trim($username))>0 && strlen(trim($password))>0 && strlen(trim($email))>0 && $emain_check>0 && $username_check>0 && $password_check>0)
        {
            ...
            {
                $status='1';
                if(SMTP_CONNECTION > 0 && $side == 'front' )
                {
                    $status='0';
                }
                /*Inserting user values*/
                $email_active_code=md5($email.time());
                $sql1="INSERT INTO users(username,password,email,last_login,email_activation,status, ipaddr, created_day)VALUES(:username,:password,:email,:created,:email_activation,:status, '$ipaddr', '$created_day')";
...
function publicUserDetails() {
    ...
    $sql = "SELECT uid,tour,username,name,password, bio,email,friend_count,profile_pic,conversation_count,updates_count,profile_bg,group_count,profile_pic_status,profile_bg_position,verified,notification_created,photos_count,facebookProfile,twitterProfile,googleProfile,instagramProfile,gender FROM users WHERE uid=:uid AND status='1'";
    try {
...
function userUpdate() {
    ...
    $sql = "SELECT uid FROM users WHERE (username=:username or email=:email) and uid!=:uid ";
    try {
        //$key=md5(SITE_KEY.$data->uid);
        if( $uid > 0)
        {
            ...
            if($count==0)
            {
                $time=time();
                if(trim($password) == ''){
                    $sql1 = "UPDATE users SET username=:username, email=:email WHERE uid =:uid";
                    
                }
                else{
                    $sql1 = "UPDATE users SET username=:username, email=:email, password=:password WHERE uid =:uid";    
                }
                ...
                $db = null;
                echo '{"usernameUpdate": [{"status":"'.$uid.'-'.$count.'"}]}';
            }
        }
    ...
    echo '{"usernameUpdate": [{"status":"'.$password.'"}]}';
}
...
/*function groupUpdates()
{
    ...
    try {
        $db = getDB();
        $sql4="UPDATE groups SET group_name = '$group_name', group_desc = '$group_desc' WHERE group_id=:group_id";
        $stmt4 = $db->prepare($sql4);
        $stmt4->bindParam("group_id", $group_id,PDO::PARAM_INT);
        $stmt4->execute();
        echo 'true';
