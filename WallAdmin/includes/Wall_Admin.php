...
class Wall_Admin
{
    ...
    {
        $db = getDB();
        $stmt2 = $db->prepare("SELECT uid,username,email,friend_count,profile_pic,conversation_count,updates_count,profile_bg,group_count,
        profile_pic_status,verified,provider,name, ipaddr, created_day  FROM users WHERE status='1' ORDER BY uid DESC LIMIT :start, :per_page");
	...
	$db = getDB();
	$stmt2 = $db->prepare("SELECT uid,username,email,friend_count,profile_pic,conversation_count,updates_count,profile_bg,group_count,
	profile_pic_status,verified,provider,name, ipaddr, created_day  FROM users WHERE status='1' AND (email  LIKE :searchKey OR  name  LIKE :searchKey OR  username  LIKE :searchKey ) ORDER BY uid DESC");  
	$searchKey="%".$searchKey."%";
    ...
    {
        $db = getDB();
        $stmt = $db->prepare("SELECT uid,username,email,friend_count,profile_pic,conversation_count,updates_count,profile_bg,group_count,
    ...
    {
        $db = getDB();
        $stmt = $db->prepare("SELECT M.msg_id,M.message,U.username,M.ip  FROM messages M, users U WHERE U.uid=M.uid_fk  ORDER BY msg_id DESC LIMIT :start, :per_page");
        $stmt->bindValue(':start', (int) trim($start), PDO::PARAM_INT);
        $stmt->bindValue(':per_page', (int) trim($per_page), PDO::PARAM_INT);
        $stmt->execute();
        $row = $stmt->fetchAll(PDO::FETCH_OBJ);
    ...
    {
	$db = getDB();
	$stmt2 = $db->prepare("SELECT M.msg_id,M.message,U.username,M.ip  FROM messages M, users U WHERE U.uid=M.uid_fk AND (M.message LIKE :searchKey OR U.username LIKE :searchKey) ORDER BY msg_id DESC");  
	$searchKey="%".$searchKey."%";
	$stmt2->bindValue(':searchKey', $searchKey, PDO::PARAM_STR);
	$stmt2->execute();
	$row = $stmt2->fetchAll(PDO::FETCH_OBJ);
    ...
    {
        $db = getDB();
        $stmt = $db->prepare("SELECT C.com_id,C.comment,U.username, C.ip FROM comments C, users U WHERE U.uid=C.uid_fk   ORDER BY com_id DESC LIMIT :start, :per_page");
        $stmt->bindValue(':start', (int) trim($start), PDO::PARAM_INT);
        $stmt->bindValue(':per_page', (int) trim($per_page), PDO::PARAM_INT);
        $stmt->execute();
        $row = $stmt->fetchAll(PDO::FETCH_OBJ);
